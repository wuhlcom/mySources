...
public class AesCmacTool {
    ...
    private byte[] genUpdateBytes(String appeui, String appnonce) {
        ...
        byte[] euiBs = MspPacketUtil.parseHexStr2Byte(appeui);
        byte[] nonceBs = MspPacketUtil.parseHexStr2Byte(appnonce);
        //按大字节序存放EUI
        for (int i = 0; i < 8; i++) {
    ...
    public String genChallengeKey(String appAuthKey, String appEUI, int nonce) throws Exception {
        ...
        byte[] masterKey = MspPacketUtil.parseHexStr2Byte(appAuthKey);
        ...
        String retStr = MspPacketUtil.parseByte2HexStr(ret);
        return retStr;
    }
